...
class Kick(commands.Cog):
    ...
    async def kick_prefix(self, ctx, member: nextcord.Member, reason: str = None):
        if not ctx.author.guild_permissions.kick_members:
            await ctx.send("This command requires `kick permission`", ephemeral=True)
            return
        ...
        # Send a direct message to the kicked member
        try:
            await member.send(f"You have been kicked 🦶 from **{ctx.guild.name}** for the **{reason}**")
        except nextcord.HTTPException:
            await ctx.send("Failed to send a direct message to the kicked member.", ephemeral=True)
    #=============================================================================================================================================================
